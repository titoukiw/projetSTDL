...
public class RecordTypeImpl implements RecordType, Scope<FieldDeclaration> {
    ...
    public boolean equalsTo(Type _other) {
        ...
        if(_other instanceof RecordTypeImpl) {
        	RecordTypeImpl other = (RecordTypeImpl) _other;
            for(FieldDeclaration field : this.fields) {
                _local = _local & (other.fields.contains(field));
            }
            for(FieldDeclaration field : other.fields) {
                _local = _local & (this.fields.contains(field));
            }
            return (_local);
        }
        return false;
        ...
        if(_other instanceof RecordTypeImpl) {
            ...
            for(FieldDeclaration field : this.fields) {
                if(other.fields.contains(field)){
                    _local = _local & field.getType().compatibleWith(other.fields.get(fields.indexOf(field)).getType());
                }else{
                    _local = false;
                }
            }
